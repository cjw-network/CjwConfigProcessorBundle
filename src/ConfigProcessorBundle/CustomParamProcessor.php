...
class CustomParamProcessor
{
     ...
     */
    public function getCustomParameters (array $customParameterKeys, array $processedParameters): array
    {
        $customParameters = [];
     ...
     */
    private function getParameterThroughParts (array $keyParts, array $processedParameters, bool $withinCustomArray = false): array
    {
     ...
     */
    private function addSiteAccessParametersBackIntoStructure (array $parameters, array $comparisonParameters): array
    {
        $indexOfCurrentHighestAccess = 0;
        foreach ($parameters as $parameterKey => $parameterValue) {
            if (
                ...
                $indexOfCurrentHighestAccess =
                    ($currentAccessIndex < $indexOfCurrentHighestAccess)? $indexOfCurrentHighestAccess : $currentAccessIndex;
                $results = $this->buildFullParameterKeys($parameterValue);

                foreach ($results as $resultKey => $resultValue) {
                    if (
                        $currentAccessIndex < $indexOfCurrentHighestAccess &&
     ...
     * @param string|null $predecessorKeys An optional key which states what key came in the level above the current.
     *
     ...
     */
    private function buildFullParameterKeys (array $parameters, string $predecessorKeys = null): array
    {
        $result = [];
     ...
     *
     * <br>Functions similarly to array_replace_recursive of php.
     *
